...
class StyleUtils {
    ...
    static getBpmnIsParallelEventBasedGateway(style) {
        return (mxgraph.mxUtils.getValue(style, StyleIdentifier.BPMN_STYLE_EVENT_BASED_GATEWAY_KIND, ShapeBpmnEventBasedGatewayKind.Exclusive) == ShapeBpmnEventBasedGatewayKind.Parallel);
    }
}
...
class EventShape extends mxgraph.mxEllipse {
    constructor(bounds, fill, stroke, strokewidth) {
        ...
        this.iconPainters = new Map([
            ...
            [ShapeBpmnEventKind.CANCEL, (paintParameter) => this.iconPainter.paintXCrossIcon(Object.assign(Object.assign({}, paintParameter), { ratioFromParent: 0.78 }))],
            [
                ShapeBpmnEventKind.ESCALATION,
    ...
    buildFlowNodeBpmnElements(processId, bpmnElements, kind) {
        ensureIsArray(bpmnElements).forEach(bpmnElement => {
            ...
            if (ShapeUtil.isEvent(kind)) {
                ...
                shapeBpmnElement = new ShapeBpmnEventBasedGateway(bpmnElement.id, eventBasedGatewayBpmnElement.name, processId, eventBasedGatewayBpmnElement.instantiate, ShapeBpmnEventBasedGatewayKind[eventBasedGatewayBpmnElement.eventGatewayType]);
            }
            else {
...
export { BpmnElementsRegistry, BpmnVisualization, FitType, IconPainter, IconPainterProvider, MarkerIdentifier, ShapeBpmnCallActivityKind, ShapeBpmnElementKind, ShapeBpmnEventBasedGatewayKind, ShapeBpmnEventKind, ShapeBpmnMarkerKind, ShapeBpmnSubProcessKind, ShapeUtil, StyleConfigurator, StyleDefault, StyleIdentifier, bpmnEventKinds, buildPaintParameter, computeScaledIconSize, mxConstants, supportedBpmnEventKinds };
